...
async def main():
    ...
    save_preprocessed_data(all_data, 'preprocessed_data.txt')
...
def label_data_for_conll(messages):
    ...
    for msg in messages:
        ...
        for token in tokens:
            ...
            # Append token and label in two separate columns
            labeled_data.append(f"{token}\t{entity_label}")
       
        labeled_data.append("")  # Separate messages with a blank line

    with open('labeled_preprocessed_data.conll', 'w', encoding='utf-8') as f:
        f.write('\n'.join(labeled_data))
